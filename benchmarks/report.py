...
def render_readme_latency(groups: list[Group], *, chinese: bool = False) -> str:
    """The latency block for `README.md` / `README.zh-CN.md`.
    ...
    The Chinese README had the same defect, which is why this is generated into both files
    instead of only the English one.
    """
    ...
    if chinese:
        lines.append(
            "由 [`benchmarks/report.py`](benchmarks/report.py) 从 "
            "[`benchmarks/results/`](benchmarks/results/) 的原始 JSON 生成；"
            "**整行取自同一个配置**（torch 在本机的默认线程数，每个 vCPU 一个），样本 p50，单进程，仅请求内批处理。"
        )
        lines.append("")
        lines.append(
            "**这是延迟，不是吞吐。** 每个请求的问题共享同一个 `state`，这是容易的情况。"
            "跨请求批处理随后已经测过，结论是在 CPU 上**不提升吞吐**（见下面的批处理一节与 "
            "[`docs/design-review.md`](docs/design-review.md) §4-M5）。"
        )
    else:
        lines.append(
        ...
        lines.append(
            ...
            "throughput on CPU -- see the batching section below and "
            "[`docs/design-review.md §4-M5`](docs/design-review.md)."
        )
    lines.append("")
...
def render_readme_dtype(groups: list[Group], *, chinese: bool = False) -> str:
    """The dtype-comparison block for the READMEs.
    ...
    if chinese:
        lines.append("| 引擎 | dtype | 3 个问题 | 相对 | 与原答案对比 |")
    else:
    ...
    if chinese:
        lines.append(
            ...
            "所以倍数是数量级结论而不是统计量；对比列由记录的答案正文算出，不是旁边的散文。"
        )
    else:
        lines.append(
...
def render_batching_readme(gains: list[BatchGain], *, chinese: bool = False) -> str:
    """A compact batching block for the READMEs: the finding, not the whole table.

    The READMEs stay short; the per-batch-size table and the multiprocess comparison live
    in `docs/design-review.md §4-M5` and `benchmarks/RESULTS.md`. What belongs in a README
    is the answer to "should I expect this to be fast", and the answer is a number.
    """
    if not gains:
        return ""
    ...
    if chinese:
        lines.append(
            ...
            "[`docs/design-review.md`](docs/design-review.md) §4-M5。批是**直接调用引擎**合成的，"
        ...
        lines.append(
            ...
            "[`docs/design-review.md §4-M5`](docs/design-review.md). Batches are synthesised by calling "
...
def targets(groups: list[Group], gaps: list[str]) -> list[tuple[Path, str | None, str | None, str]]:
    ...
    return [
        ...
        (
            ...
            BATCHING_BEGIN,
            BATCHING_END,
            render_batching_readme(load_batch_gains(), chinese=False),
        ),
        (
            ROOT / "README.zh-CN.md",
            README_LATENCY_BEGIN,
            README_LATENCY_END,
            render_readme_latency(groups, chinese=True),
        ),
        (
            ROOT / "README.zh-CN.md",
            README_DTYPE_BEGIN,
            README_DTYPE_END,
            render_readme_dtype(groups, chinese=True),
        ),
        (
            ROOT / "README.zh-CN.md",
            BATCHING_BEGIN,
            BATCHING_END,
            render_batching_readme(load_batch_gains(), chinese=True),
        ),
        (
