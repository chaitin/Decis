...
三个游戏的阅读顺序是一样的。游戏栏带标题、实时状态、得分和操作按钮；它下面是棋盘和模型读到的东西
并排——恐龙除外：它的画布是横向的，所以游戏画面占满整行，模型接下来要做的动作放在它下面。
再往下是一行两块面板：推理面板，和"最近一次调用"的控制台。控制台**默认展开**，左边是请求、右边是
响应；只想看棋盘的话，点一下就能折成一行。
...
控制台印的是最近一次 `/v1/systemone` 调用**发出去时的请求体**，旁边就是响应或错误。
