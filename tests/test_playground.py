...
def test_the_app_bar_is_the_same_on_every_page() -> None:
    ...
    for name in PAGES:
        ...
        # Two things may differ, and only two: which nav link is the current page, and the
        # index page's own id on the chip (the games use the shared one).
        head = head.replace(' aria-current="page"', "").replace(' id="status-chip"', "")
        heads[name] = head
    assert len(set(heads.values())) == 1, "the app bar differs between pages"
    # ...and it names the page you are on, which is the only thing that may differ. The index
    # is the home page, so none of its links is "current".
    for name in GAMES:
        text = (WEB / name).read_text(encoding="utf-8")
