...
**These are not throughput figures.** `laya-*-sweep.json` was measured against the engine before the Decis server existed, and cross-request batching -- the mechanism [`docs/design.md §6`](docs/design.md) relies on for throughput -- is not implemented yet, so nothing here can be quoted as QPS. See [`benchmarks/README.md`](benchmarks/README.md) M4/M5.
