...
**These are latencies, not throughput.** Every question in a row shares one `state`, which is the easy case. Cross-request batching was measured and does **not** raise throughput on CPU -- see [`docs/performance.md`](docs/performance.md) and [`docs/design-review.md §4-M5`](docs/design-review.md).
